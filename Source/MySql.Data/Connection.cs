    ...
    public sealed partial class MySqlConnection : IDisposable
    {
      ...
      Dispose();
        ...
        public override void Open()
        {
            ...
            {
                // Dealing with bug between this and AWS Aurora that incorrectly results 
                // in an "Unknown database" error from the server. Reset the connection and 
                // try the command again.
                if (ex.Message.StartsWith("Unknown database") && changeDbErrorCount++ == 0)
                {
                    this.driver.Reset();
                    goto change_database;
                }
